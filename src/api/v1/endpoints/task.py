...
async def create_task(
    ...
    return await crud_task.create(
        db=db, create_schema=create_data, creator_id=current_user.id
    )
...
async def update_task(
    ...
    task = await crud_task.update(
        db=db, task_id=task_id, update_schema=update_data
    )
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Task not found"
...
async def delete_task(
    ...
    if not task.creator == current_user and not current_user.is_admin:
        raise HTTPException(
            ...
            detail="You dont have right to delete this task",
        )
