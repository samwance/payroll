...
async def create_user(
    ...
    user = await crud_user.create(
        db=db, create_schema=create_data, password=password
    )
...
@router.get(
    "/users/{user_id}/", response_model=Union[UserResponse, UserFullResponse]
)
async def read_user(
    user_id: int,
    current_user: User = Depends(get_current_user),
...
async def update_user(
    ...
    return await crud_user.update(
        db=db, user_id=current_user.id, update_schema=update_data
    )
